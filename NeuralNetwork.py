...
class NeuralNetworkClassifier:
    ...
    def adam(self, X_train, y_train, X_val, y_val, epochs, eta, n_examples, batchSize):
        ...
        while (t < epochs):
            ...
            while (mini < (n_examples / batchSize)):
                ...
                mini += 1
            _, _, yhat = self.forwardPropagation(X_train)
            print("loss after epoch ", self.loss_function(yhat, y_train, X_train))
            t += 1
            self.update_loss_accuracy(X_train, X_val, y_train, y_val)
    ...
    def fit(self, x, y, batch_size=BATCH_SIZE, epochs=100, eta=0.01):
        ...
        if self.optimizer == 'sgd':
            self.sgd(x, y, epochs, eta)
        elif self.optimizer == 'momentum':
            # TODO
            None
        elif self.optimizer == 'nesterov':
            self.nag(x, y, epochs, eta)
        elif self.optimizer == 'rmsprop':
            # TODO
            None
        elif self.optimizer == 'adam':
            self.adam(x, y, epochs, eta, x.shape[0], batch_size)
        elif self.optimizer == 'nadam':
            # TODO
            None
        elif self.optimizer == None:
            self.vanillaGradDescent()
        else:
            raise Exception("Unsupported optmizer.")
